import {
  reduxForm
} from 'redux-form';

import UserForgotPasswordComponent from '../../components/user/ForgotPasswordComponent';
import {
  resetPassword,
  RESET_PASSWORD_SUCCESS,
  RESET_PASSWORD_FAILURE
} from '../../actions/user/ForgotPasswordAction';

//Client side validation
function validate(values) {
  var errors = {};
  var hasErrors = false;
  if (!values.email || values.email.trim() === '') {
    errors.email = 'The email field is required.';
    hasErrors = true;
  }
  return hasErrors && errors;
}

const mapDispatchToProps = (dispatch) => {
  return {
    userResetPasswordAPI: (formData, dispatch) => {
      return new Promise((resolve, reject) => {
        dispatch(resetPassword(formData)).then((response) => {
          let data = response.payload.data;

          // if any one of these exist, then there is a field error
          if (!data.status) {
            // let other components know of error by updating the redux` state
            console.log(response.payload);
            dispatch({
              type: RESET_PASSWORD_FAILURE,
              payload: response.payload,
              message: RESET_PASSWORD_FAILURE,
            });

            reject(data);
          } else {

            // let other components know that we got user and things are fine by updating the redux` state
            dispatch({
              type: RESET_PASSWORD_SUCCESS,
              payload: response.payload,
              message: RESET_PASSWORD_SUCCESS,
            });

            // this is for redux-form itself
            resolve(data);
          }
        });
      });
    },
  }
}

/*
const mapDispatchToProps = (dispatch) => {
  return {
   forgotPwd: validateAndForgotPwd,
   resetMe: () =>{
    //sign up is not reused, so we dont need to resetUserFields
    //in our case, it will remove authenticated users
     // dispatch(resetUserFields());
    }
  }
}
*/

function mapStateToProps(state, ownProps) {
  return {
    user: state.user
  };
}


// connect: first argument is mapStateToProps, 2nd is mapDispatchToProps
// reduxForm: 1st is form config, 2nd is mapStateToProps, 3rd is mapDispatchToProps
export default reduxForm({
  form: 'ForgotPwdForm',
  fields: ['email'],
  null,
  null,
  validate
}, mapStateToProps, mapDispatchToProps)(UserForgotPasswordComponent);
